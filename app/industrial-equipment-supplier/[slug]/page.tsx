  ...
  return (
    <>
      ...
      <div className="relative">
        <section className="relative min-h-[180px] overflow-hidden sm:min-h-[34vh]">
          <Image
          ...
          <Navbar />
          <div className="relative z-10 mx-auto flex min-h-[180px] w-full max-w-7xl flex-col justify-end px-4 pb-5 pt-20 sm:min-h-[34vh] sm:px-6 sm:pb-10 sm:pt-32 lg:px-8">
            ...
            <h1 className="max-w-4xl text-xl font-extrabold leading-snug tracking-tight text-white sm:text-4xl sm:leading-tight">
              {product.title}
            </h1>
          </div>
        </section>
      ...
      <section className="bg-[#f7f8fa] py-5 pb-[calc(5.5rem+env(safe-area-inset-bottom))] sm:py-12 sm:pb-12">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <p className="mb-4 break-words text-xs text-[#555] sm:mb-6 sm:text-sm">
            <Link href="/industrial-equipment-supplier" className="font-semibold text-[#f0571f] hover:underline">
              Industrial Equipment Supplier
            </Link>
            <span className="mx-2">/</span>
            <span className="inline">{product.title}</span>
          </p>

          <div className="grid min-w-0 gap-5 sm:gap-8 lg:grid-cols-12">
            <div className="min-w-0 lg:col-span-7">
              <ProductDetailClient product={product} />
              <div className="mt-4 min-w-0 rounded-2xl border border-zinc-200 bg-white p-4 sm:mt-6 sm:p-6">
                ...
                <dl className="mt-4 overflow-hidden rounded-xl border border-zinc-100 sm:mt-5">
                  {product.specs.map((spec) => (
                    <div key={spec.label} className="grid grid-cols-2 border-b border-zinc-100 last:border-b-0">
                      <dt className="break-words bg-zinc-50 px-2.5 py-2 text-[11px] font-semibold leading-snug text-[#555] sm:px-4 sm:py-3 sm:text-sm">
                        {spec.label}
                      </dt>
                      <dd className="break-words px-2.5 py-2 text-[11px] leading-snug text-[#333] sm:px-4 sm:py-3 sm:text-sm">
                        {spec.value}
                      </dd>
                    </div>
                  ))}
              ...
              <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-2 sm:gap-4 lg:grid-cols-4">
                {related.map((item) => (
                  <Link
                    ...
                    className="min-w-0 overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md sm:rounded-2xl"
                  >
