...
function setGoogleTranslateCookie(language: string) {
  ...
  if (!["localhost", "127.0.0.1"].includes(window.location.hostname)) {
    document.cookie = `googtrans=${value}; path=/; max-age=${maxAge}; domain=.${window.location.hostname}`;
  }
}

function applyGoogleTranslateLanguage(language: string) {
...
export default function TopBar() {
  ...
  useEffect(() => {
    window.googleTranslateElementInit = () => {
      ...
      new window.google.translate.TranslateElement(
        {
          autoDisplay: false,
          includedLanguages: languages.map((item) => item.value).join(","),
          pageLanguage: "en",
        },
  ...
  useEffect(() => {
    ...
    const retryDelays = [0, 80, 200, 450, 800];
    const retryIds = retryDelays.map((delay) =>
      window.setTimeout(() => applyGoogleTranslateLanguage(language), delay),
    );
  ...
  function changeLanguage(nextLanguage: string) {
    ...
    if (applyGoogleTranslateLanguage(nextLanguage)) return;
