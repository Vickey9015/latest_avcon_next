...
export default function ProductCard({ product, onContact, compact = false }: ProductCardProps) {
  ...
  return (
    ...
    >
      <div className="relative aspect-[4/3] bg-zinc-50 sm:aspect-[5/3] lg:aspect-[4/3]">
        ...
        {images.length > 1 ? (
          <>
            <button
              type="button"
              onClick={prev}
              className="absolute left-1 top-1/2 z-10 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-full bg-white/95 text-[#1a1a1a] shadow touch-manipulation sm:left-2 sm:h-8 sm:w-8"
            ...
            <button
              type="button"
              onClick={next}
              className="absolute right-1 top-1/2 z-10 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-full bg-white/95 text-[#1a1a1a] shadow touch-manipulation sm:right-2 sm:h-8 sm:w-8"
      ...
      <div className="flex flex-1 flex-col p-2.5 sm:p-4">
        <p className="truncate text-[9px] font-bold uppercase tracking-wide text-[#f0571f] sm:text-[11px]">
          {getCategoryLabel(product.categoryId)}
        </p>
        <Link
          href={productHref(product.slug)}
          className="mt-1 line-clamp-2 min-h-[2.4em] text-[13px] font-bold leading-snug text-[#1e3a5f] hover:text-[#f0571f] sm:min-h-0 sm:text-base"
        >
          {product.title}
        </Link>
        ...
        <button
          ...
          className="mt-2 flex h-9 w-full items-center justify-center gap-1.5 rounded-lg bg-[#f0571f] text-[11px] font-bold text-white transition hover:bg-[#d94818] touch-manipulation sm:mt-3 sm:h-10 sm:gap-2 sm:text-sm"
        >
          <svg className="h-3.5 w-3.5 sm:h-4 sm:w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <p className="mt-2 hidden text-xs leading-5 text-[#555] sm:mt-3 sm:line-clamp-3 sm:block sm:text-sm sm:leading-6">
